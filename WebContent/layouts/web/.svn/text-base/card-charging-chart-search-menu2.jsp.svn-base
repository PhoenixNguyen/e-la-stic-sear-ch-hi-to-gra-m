	...
	$(document).ready(function(){
		...
		$.each(keys, function(k , v){
			...
			$('.scope_search .'+v+'').live('click', function(){
				...
				//Set filters
				if(v == 'merchant'){
					 $('form[name=filter_histogram] input[name=filter_merchant]').val(item);
				}
				if(v == 'paymentProvider'){
					 $('form[name=filter_histogram] select[name=filter_provider] option[selected]').prop('selected', false);
					 $('form[name=filter_histogram] select[name=filter_provider] option[value='+item+']').prop('selected', true);
				}
				if(v == 'type'){
					 $('form[name=filter_histogram] input[name=filter_card_type]').removeAttr('checked')
					 $('form[name=filter_histogram] input[name=filter_card_type][value='+item+']').attr('checked', 'checked');
				}
		...
		$('.scope_search .filter').live('click', function(){
			...
			//reset
			if(child == 'merchant'){
				$('form[name=filter_histogram] input[name=filter_merchant]').val('');
			}
			if(child == 'paymentProvider'){
				 $('form[name=filter_histogram] select[name=filter_provider] option[selected]').prop('selected', false);
			}
			if(child == 'type'){
				$('form[name=filter_histogram] input[name=filter_card_type]').removeAttr('checked')
